...
def test_hachage() -> str :
	...
	#1.encodage
	b_texte = b.encode('utf-8')
	
	#2.hachage
	hash_texte = hashlib.sha256(b_texte)
	
	#3.conversion
	empreinte = hash_texte.hexdigest()
...
def hack() -> str :
	...
	#1.encodage
	a_texte = a.encode('utf-8')
	
	#2.hachage
	hash_texte = hashlib.sha256(a_texte)
	
	#3.conversion
	emprt = hash_texte.hexdigest()
...
b1 = Block(0, time(), "Alice", "1")
print(b1)

class Blockchain:
	def __init__(self):
		self.chain = []
		genesis_block = Block(0, time(), "Alice", "1")
	...
	def is_chain_valid(self):
		for i in range(1, len(self.chain)):		
			...
			if current_block.hash != current_block.calculate_hash():
				print(f"Block{i} : invalide hash")
				return False
			
			if current_block.previous_hash != previous_block.hash:
				print(f"Block{i} : brisée chaine")
				return False
